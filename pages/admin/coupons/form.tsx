...
const CouponForm = ({
  ...
  const [discountType, setDiscountType] = useState<ICoupon['discountType']>(
  ...
  const [applicableTo, setApplicableTo] = useState<ICoupon['applicableTo']>(
    initialData?.applicableTo || 'all',
  )
  const [applicableItems, setApplicableItems] = useState(
    initialData?.applicableItems?.join(', ') || '',
  )
  const [minPurchaseAmount, setMinPurchaseAmount] = useState(
    initialData?.minPurchaseAmount || 0,
  )
    ...
    >
      ...
      <div>
        ...
        <select
          ...
          value={discountType}
          onChange={(e) =>
            setDiscountType(e.target.value as ICoupon['discountType'])
          }
          required
        >
      ...
      <div>
        ...
        <select
          ...
          value={applicableTo}
          onChange={(e) =>
            setApplicableTo(e.target.value as ICoupon['applicableTo'])
          }
          required
        >
      ...
      {(applicableTo === 'products' || applicableTo === 'categories') && (
        <div>
          <label
            htmlFor="applicableItems"
            className="block text-sm font-medium text-gray-700"
          >
            IDs de Productos o Slugs de Categorías (separados por comas)
          </label>
          <input
            type="text"
            id="applicableItems"
