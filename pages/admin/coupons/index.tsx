...
export default function AdminCouponsIndex() {
  ...
  return (
    <AdminLayout>
      ...
      <Link href="/admin/coupons/create" className="bg-pink-500 text-white px-4 py-2 rounded-md hover:bg-pink-600 transition mb-6 inline-block">
        Crear Nuevo Cupón
      </Link>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aplica a</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {coupons.map((coupon) => (
                <tr key={coupon._id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{coupon.discountType === 'percentage' ? 'Porcentaje' : 'Monto Fijo'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{coupon.discountType === 'percentage' ? `${coupon.value}%` : `$U ${coupon.value}`}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(coupon.expirationDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{coupon.usedCount} / {coupon.maxUses}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {coupon.applicableTo === 'all' ? 'Todos' : 
                     coupon.applicableTo === 'products' ? `Productos (${coupon.applicableItems?.length || 0})` : 
                     `Categorías (${coupon.applicableItems?.length || 0})`}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <Link href={`/admin/coupons/edit/${coupon.code}`} className="text-indigo-600 hover:text-indigo-900 mr-4">
                      Editar
                    </Link>
                    <button onClick={() => handleDelete(coupon.code)} className="text-red-600 hover:text-red-900">
                      Eliminar
                    </button>
                  </td>
                </tr>
