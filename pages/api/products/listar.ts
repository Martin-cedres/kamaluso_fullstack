...
export default async function handler(
  ...
  try {
    ...
    if (subCategoria)
      query.subCategoria = { $regex: new RegExp(`^${subCategoria}$`, 'i') }
    if (slug) query.slug = { $regex: new RegExp(`^${slug}$`, 'i') }
    if (_id) query._id = new ObjectId(_id)
    if (typeof destacadoFilter !== 'undefined')
      query.destacado = destacadoFilter
    ...
    const mapped = productos.map((p: any) => ({
      ...
      subCategoria: Array.isArray(p.subCategoria)
        ? p.subCategoria
        : p.subCategoria
          ? [p.subCategoria]
          : [],
