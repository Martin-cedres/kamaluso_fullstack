  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      const productoDoc: any = {
        ...
        seoKeywords: typeof fields.seoKeywords === "string" ? fields.seoKeywords.split(",").map(s => s.trim()) : [],
        alt: String(fields.alt || ""),
        notes: String(fields.notes || ""),
        status: String(fields.status || "activo"),
        destacado: (fields.destacado === "true") || (fields.destacado === true) || false,
        imageUrl,
        images: imagesUrls,
        creadoEn: new Date(),
      };
      ...
      res.status(201).json({ ok: true, mensaje: "Producto creado correctamente", id: result.insertedId });
    } catch (error) {
