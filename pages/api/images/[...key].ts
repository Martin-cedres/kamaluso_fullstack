...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!keyParts || !keyParts.length) return res.status(400).json({ error: "Falta la key" });
    ...
    if (!data.Body) return res.status(404).json({ error: "Imagen no encontrada" });
    ...
    res.status(404).json({ error: "Imagen no encontrada", detalles: String(err.message || err) });
  }
}
