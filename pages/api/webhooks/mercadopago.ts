...
const generateEmailContent = (order: any) => {
  ...
  const paymentInstructions =
    '<p><strong>Instrucciones Mercado Pago:</strong> Tu pago ha sido procesado exitosamente a través de Mercado Pago. ¡Gracias!</p>'
...
const generateAdminEmailContent = (order: any, orderId: string) => {
  ...
  const shippingInfo = generateShippingHTML(order.shippingDetails)
  const paymentInstructions =
    '<p><strong>Instrucciones Mercado Pago:</strong> El pago ha sido procesado exitosamente a través de Mercado Pago.</p>'
...
export default async function handler(
  ...
  try {
    ...
    console.log(
      `Respuesta de MP recibida. Estado: ${mpPayment.status}, External Ref: ${mpPayment.external_reference}`,
    )
    ...
    if (!mpPayment.external_reference) {
      ...
      return res
        .status(400)
        .json({ message: 'Payment does not have an external reference.' })
    }
    ...
    if (order.status === 'pagado') {
      console.log(
        `Advertencia: La orden ${orderId} ya fue marcada como pagada.`,
      )
    ...
    if (mpPayment.status === 'approved') {
      console.log(
        `4. El pago fue aprobado. Actualizando la orden ${orderId} a 'pagado'.`,
      )
      ...
      if (updateResult.modifiedCount === 0) {
        console.log(
          `Advertencia: No se modificó la orden ${orderId}. Puede que ya estuviera actualizada.`,
        )
        return res
          .status(200)
          .json({ message: 'Order status was not modified.' })
      }
      ...
      console.log(
        `--- WEBHOOK MERCADO PAGO COMPLETADO PARA ORDEN ${orderId} ---`,
      )
    } else {
      console.log(
        `El estado del pago es '${mpPayment.status}'. No se actualiza la orden.`,
      )
    }
