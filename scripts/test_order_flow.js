...
if (!process.env.MONGODB_URI || !emailUser || !emailPass) {
    console.error("Error: Asegúrate de que MONGODB_URI, EMAIL_SERVER_USER, y EMAIL_SERVER_PASSWORD estén definidos en tu archivo .env.local");
    process.exit(1);
}
...
async function runTest() {
    ...
    try {
        ...
        console.log("Pedido recuperado de la BD:", JSON.stringify(retrievedOrder, null, 2));
        ...
        if (!retrievedOrder.paymentMethod) missingFields.push("paymentMethod (metodoPago)");
        ...
        console.log(`Correo de prueba enviado a ${retrievedOrder.email}. Message ID: ${info.messageId}`);
        console.log("\nResultado: ¡ÉXITO! Nodemailer se configuró correctamente y el correo fue enviado.");
        console.log("Por favor, revisa la bandeja de entrada de 'comprador.prueba@example.com' para confirmar la recepción.");
