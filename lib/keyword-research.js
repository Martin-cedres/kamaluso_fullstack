...
const gemini_agent_1 = require("./gemini-agent");
/**
 ...
 */
async function getGoogleSuggestions(query, lang = 'es') {
    if (!query) {
        return [];
    }
 ...
 */
async function getSearchTrends(productName, categoryName) {
    ...
    `;
        const dynamicTrendsSummary = await (0, gemini_agent_1.generateWithFallback)(analysisPrompt);
        return {
