 ...
 */
const ProductSchema = ({ product, averageRating, reviewCount, reviews }) => {
  if (!product) {
    return null;
  }
  ...
  // Mapear reseñas al formato Schema.org si existen
  const mappedReviews = reviews && Array.isArray(reviews) ? reviews.map(review => ({
    '@type': 'Review',
    author: {
      '@type': 'Person',
      name: review.user?.name || 'Cliente de Kamaluso',
    },
    ...
    '@graph': [
      {
        ...
        // Añadir las reseñas individuales si existen
        ...
        : []),
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: 'Productos',
            item: `${siteConfig.baseUrl}/productos`,
          },
          {
            '@type': 'ListItem',
            position: 3,
